...
--1)
SELECT A.[CODIGO], A.[NOMBRE], A.[Nacionalidad] 
FROM [BIBLIOTECA].[AUTOR] AS A
		JOIN [BIBLIOTECA].[AUTOR_LIBRO] AS AL ON A.CODIGO=AL.CODIGO_AUTOR
...
, [edad]
FROM BIBLIOTECA.USUARIO
WHERE EDAD> (SELECT AVG(EDAD) AS PROMEDIO FROM BIBLIOTECA.USUARIO)
